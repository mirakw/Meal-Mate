...
class GroceryList(db.Model):
    ...
    id = db.Column(db.String, primary_key=True, default=lambda: str(__import__('uuid').uuid4()))
...
def auth_login():
    try:
        # Try to get REPL_ID for real OAuth
        repl_id = os.environ.get('REPL_ID')
        if repl_id:
            # Try real Google OAuth through Replit
            from replit_auth import make_replit_blueprint
            try:
                replit_bp = make_replit_blueprint()
                app.register_blueprint(replit_bp, url_prefix="/auth", name='replit_auth_bp')
                return redirect('/auth/replit_auth/login')
            except Exception as oauth_error:
                logging.error(f"OAuth setup failed: {oauth_error}")
                # Fall through to demo user creation
        
        # Create demo user for immediate testing
        ...
        logging.info("Demo user logged in successfully")
        return redirect('/')
...
def auth_callback():
    # Handle OAuth callback or errors
    error = request.args.get('error')
    if error:
        logging.error(f"OAuth error: {error} - {request.args.get('error_description')}")
    
    # Always redirect to home - OAuth success will be handled by replit_auth
    return redirect('/')
...
def delete_folder(folder_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({'error': 'Cannot delete folder or folder not found'}), 400
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def get_folder_recipes(folder_id):
    ...
    try:
        folder_path = os.path.join(f"user_data/{current_user.id}/saved_recipes", folder_id)
...
def get_recipes():
    ...
    try:
        ...
        for folder in folders:
            folder_path = os.path.join(f"user_data/{current_user.id}/saved_recipes", folder.id)
...
def get_recipe_details(folder_id, recipe_name):
    ...
    try:
        ...
        filename = "".join(c if c.isalnum() else "_" for c in recipe_name).lower()
...
def extract_recipe():
    ...
    try:
        ...
        if recipe:
            ...
            os.makedirs(user_dir, exist_ok=True)
            filepath = save_recipe_to_file(recipe, directory=f"user_data/{current_user.id}/saved_recipes", folder_id=folder_id)
            ...
            return jsonify({'error': 'Website blocked scraping. Try a different recipe URL or enter the recipe manually.'}), 400
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    ...
    try:
        ...
        for recipe_name in recipe_names:
            if recipe_name in all_recipes:
                ...
                return jsonify({'error': f'Recipe "{recipe_name}" not found'}), 400
...
def delete_recipe(folder_id, recipe_name):
    ...
    try:
        ...
        user_folder_manager._update_recipe_counts()
        return jsonify({'success': True, 'message': f'Recipe "{recipe_name}" deleted successfully'})
...
def move_recipe():
    ...
    if current_folder == target_folder:
        return jsonify({'error': 'Recipe is already in the target folder'}), 400
    
    try:
        # Find the actual recipe file by searching for the recipe name in the folder
        source_folder_path = os.path.join(f"user_data/{current_user.id}/saved_recipes", current_folder)
        ...
        target_dir = os.path.join(f"user_data/{current_user.id}/saved_recipes", target_folder)
        ...
        return jsonify({
            'success': True,
            'message': f'Recipe "{recipe_name}" moved successfully from {current_folder} to {target_folder}'
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def get_grocery_lists():
    ...
    try:
        grocery_lists = GroceryList.query.filter_by(user_id=current_user.id).order_by(GroceryList.created_at.desc()).all()
...
def save_grocery_list():
    ...
    try:
        ...
        if not data or 'groceryList' not in data or 'mealPlan' not in data:
            return jsonify({'error': 'Missing required data: groceryList and mealPlan'}), 400
...
def recipe_search():
    ...
    try:
        data = request.get_json()
        search_term = data.get('description', '').strip() or data.get('search_term', '').strip()
...
def save_search_result():
    ...
    try:
        ...
        if recipe_data:
            ...
            print(f"Saving complete recipe data: {recipe_data.get('name')} with {len(recipe_data.get('ingredients', []))} ingredients")
        else:
            ...
            try:
                ...
                if extracted_recipe and extracted_recipe.ingredients:
                    recipe_data = {
                        ...
                        'serving_size': extracted_recipe.serving_size or 'See original recipe'
                    }
                else:
                ...
                recipe_data = {
                    ...
                    'serving_size': 'See original recipe'
                }
